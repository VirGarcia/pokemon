...
// Define la variable src según el dominio
let src;
if (dominioActual === 'localhost') {
	src = "http://localhost/pokemonVir/webService/controller/pokemon.php?";
} else {
	src = "https://"+dominioActual+"/pokemonVir/webService/controller/pokemon.php?";
}

//var src="http://localhost/pokemonVir/webService/controller/pokemon.php?";
...
function obtieneTodos() {
	...
	let srcOperacion=src+"op=GetAll";

	xhr.open("GET", srcOperacion, true); // Hacemos la petición por POST
...
function enviaInfo(valor1, valor2, valor3){
	...
	let srcOperacion = src + "op=Insert";
...
function borraUno(identificador) {
	...
	let srcOperacion = src + "op=Delete";
